"""
Fixed GUI Interface - Handles Maximize After Minimize
Properly restores and arranges windows after minimizing
"""
...
class CleanGUI:
    def __init__(self, config: Dict, callbacks: Dict[str, Callable]):
        """
        Initialize the CleanGUI object
        :param config: The application configuration
        :param callbacks: A dictionary of callback functions
        """
        self.config = config
        ...
        self.prompt_text = None  # The text box for the prompt
        self.status_label = None  # The label for the status
        self.window_count_label = None  # The label for the number of windows
        self.app_icons_frame = None  # The frame for the app icons
        self.app_buttons = {}  # The buttons for each app
        
        # State
        self.history_manager = None  # The history manager
        self.active_apps = []  # The list of active apps
        
        # Colors
        self.colors = config['gui']['theme']  # The colors for the theme
    ...
    def _create_main_content(self):
        ...
        prompt_label = tk.Label(
            main_frame,
            text="Enter prompt for all AI applications:",
            font=self.config['gui']['fonts']['normal'],
            bg=self.colors['bg_primary'],
            fg=self.colors['fg_primary']
        )
    ...
    def _create_control_panel(self):
        ...
        # History button
        history_btn = self._create_button(
            buttons_row,
            "History",
    ...
    def _create_status_bar(self):
        ...
        # Hotkey hints
        hints_text = "Hotkeys: Enter=Send | Shift+Enter=New Line | Up/Down=History | Click app icons to bring to front"
    ...
    def _bind_hotkeys(self):
        ...
        self.prompt_text.bind("<Up>", self._on_up_key)
        self.prompt_text.bind("<Down>", self._on_down_key)
    ...
    def _on_up_key(self, event):
        ...
        if self.history_manager:
            prev_prompt = self.history_manager.get_previous()
            if prev_prompt is not None:
                self._set_prompt_text(prev_prompt)
        return "break"
    
    def _on_down_key(self, event):
        """Handle Down arrow for history"""
        if self.history_manager:
            next_prompt = self.history_manager.get_next()
            if next_prompt is not None:
                self._set_prompt_text(next_prompt)
        return "break"
    ...
    def _on_send_prompt(self):
        ...
        self.update_status("Sending prompt to all AI applications", "info")
        
        # Send in background thread
        def send_async():
            try:
                result = self.callbacks['send_prompt'](prompt)
                
                if result['success'] > 0:
                    status_msg = f"Sent to {result['success']}/{result['total']} applications"
                    self.update_status(status_msg, "success")
                    
                    # Clear prompt after successful send
                    self.root.after(0, self._clear_prompt_text)
                else:
                    self.update_status("No applications received the prompt", "warning")
    ...
    def _toggle_history(self):
        ...
        if self.history_manager:
            ...
            messagebox.showinfo("Input History", f"Total entries: {stats['total_entries']}")
    ...
    def _on_maximize_grid(self):
        """Handle maximize/arrange windows in grid - with restore"""
        self.update_status("Restoring and arranging windows in grid position", "info")
        
        def maximize_async():
            try:
                # First refresh window list to ensure we have all windows (including minimized ones)
    ...
    def _on_app_click(self, app_name: str):
        ...
        def bring_to_front_async():
            try:
                # First refresh windows to ensure we have current list
                self.callbacks['refresh_windows']()
