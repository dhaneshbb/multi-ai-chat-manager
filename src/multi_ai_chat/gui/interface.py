...
class CleanGUI:
    ...
    def create_gui(self):
        ...
        # Always on top behavior based on configuration
        if gui_config.get('always_on_top', False):
            self.root.attributes('-topmost', True)
            self.logger.info("Window set to always on top")
        else:
            self.root.attributes('-topmost', False)
            self.logger.info("Window set to normal behavior")
    ...
    def _create_control_panel(self):
        ...
        always_on_top_btn = self._create_button(
            buttons_row,
            "Toggle On Top",
            self._on_toggle_always_on_top,
            style='secondary',
            width=16
        )
        always_on_top_btn.pack(side=tk.LEFT, padx=(0, 10))
    ...
    def _create_status_bar(self):
        ...
        # Hotkey hints
        hints_text = "Hotkeys: Enter=Send | Shift+Enter=New Line | Select AIs above to target specific models | Click 'Toggle Always On Top' to change window behavior"
    ...
    def _on_toggle_always_on_top(self):
        ...
        button_text = "Always On Top: ON" if new_state else "Always On Top: OFF"
        # Find the button and update its text if needed
        # This could be enhanced to update button appearance
    ...
    def toggle_always_on_top(self):
        ...
        # Update config
        self.config['gui']['window']['always_on_top'] = new_state
        
        status_msg = "Window set to always on top" if new_state else "Window set to normal behavior"
    ...
    def set_always_on_top(self, always_on_top: bool):
        ...
        self.config['gui']['window']['always_on_top'] = always_on_top
        
        status_msg = "Window set to always on top" if always_on_top else "Window set to normal behavior"
        self.update_status(status_msg, "info")
        self.logger.info(status_msg)
