...
class ReliablePromptSender:
    ...
    def _send_prompt_to_window(self, hwnd: int, prompt: str) -> bool:
        """Send prompt to a specific window - exact method from working code"""
        try:
    ...
    def test_clipboard_functionality(self) -> bool:
        """Test if clipboard operations work properly"""
        try:
            ...
            if success:
                self.logger.info("Clipboard functionality test passed")
            else:
                self.logger.error("Clipboard functionality test failed")
