"""
Fixed Window Manager for Multi-AI Chat Manager
Corrected minimize/restore issue and improved grid positioning
"""
...
class WindowManager:
    def __init__(self, config: Dict):
        """
        Initialize the Window Manager with the application configuration
        ...
        # Custom app ordering for grid arrangement (priority 0-5)
        # This is used for the grid arrangement of the AI windows
        # The order is determined by the app name and priority
        self.app_priority = {
            'Claude': 0,
            'Google Gemini': 1, 
            'Perplexity': 2,
    ...
    def arrange_windows_grid(self, display: Dict) -> None:
        """Arrange windows in custom ordered grid - improved version"""
        ...
        self.logger.info(f"Arranging {len(sorted_windows)} windows in {grid_cols}x{grid_rows} grid position")
        ...
        self.logger.info(f"Successfully arranged {arranged_count}/{len(sorted_windows)} windows in grid position")
