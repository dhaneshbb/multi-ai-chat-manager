...
def load_existing_config_from_path(config_path):
    """Load existing configuration from specific path"""
    try:
...
def load_existing_config():
    """Load existing configuration if it exists (deprecated - use load_existing_config_from_path)"""
    config_paths = ["config.yml", "src/multi_ai_chat/config/config.yml"]
    
    for config_path in config_paths:
        if os.path.exists(config_path):
            return load_existing_config_from_path(config_path)
    return None

def save_config(config, config_path=None):
    """Save configuration to file"""
    if config_path is None:
        config_path = "config.yml"  # Default to root directory
...
def interactive_setup():
    ...
    print("=" * 60)
    print("This tool will help you create the required config.yml file")
    print()
    ...
    if shortcuts:
        ...
        print("You can manually add them in the configuration")
    
    # Determine config location
    config_locations = [
        "config.yml",  # Root directory (recommended)
    ...
    for config_path in config_locations:
        if os.path.exists(config_path):
            existing_config = load_existing_config_from_path(config_path)
            existing_path = config_path
            break
    
    if existing_config:
        print(f"\nFound existing config at: {existing_path}")
    ...
    print(f"\nWhere should the config.yml be created?")
    print("1. Root directory (config.yml) - Recommended")
    print("2. Development structure (src/multi_ai_chat/config/config.yml)")
    
    choice = input("Select location (1 or 2): ").strip()
    if choice == "2":
        config_path = config_locations[1]
    ...
    if save_config(config, config_path):
        ...
        print(f"Make sure the config.yml file is in the correct location when running the application")
        ...
        print(f"\nIMPORTANT: Ensure {config_path} is present when running the application")
    else:
...
def validate_config():
    ...
    if not config_found:
        ...
        print("\nRun option 1 (Interactive Setup) to create configuration")
        return False
    
    try:
        print(f"Validating configuration: {config_path}")
        config = load_existing_config_from_path(config_path)
        ...
        print(f"\nSummary: {valid_shortcuts}/{len(ai_apps)} shortcuts are valid")
        
        if valid_shortcuts == 0 and len(ai_apps) > 0:
            print("WARNING: No valid shortcuts found. Application may not work properly.")
            print("Please update the shortcut paths in your config.yml file")
